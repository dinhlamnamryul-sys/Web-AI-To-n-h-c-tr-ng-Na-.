...
st.markdown("""
<style>
    /* 1. NỀN CẦU VỒNG CHUYỂN ĐỘNG */
    .stApp {
        background: linear-gradient(120deg, #84fab0 0%, #8fd3f4 100%);
        font-family: 'Comic Sans MS', cursive, sans-serif;
    }

    /* 2. KHUNG CARD NỔI BỒNG BỀNH */
    .game-card {
        background-color: #ffffff;
        border-radius: 40px;
        padding: 30px;
        /* Bóng đổ cực đậm để tạo cảm giác card đang bay */
    ...
    /* 3. SỐ HỌC 3D BÓNG BẨY */
    .super-number {
        font-size: 160px;
        line-height: 1.2;
        font-weight: 900;
        color: #ff4757;
        /* Tạo viền chữ và bóng đổ cho số */
        text-shadow: 4px 4px 0px #ffffff, 8px 8px 0px rgba(0,0,0,0.15);
        margin: 10px 0;
        animation: pop 0.5s;
    }

    /* 4. NÚT BẤM "NHỰA DẺO" 3D (QUAN TRỌNG NHẤT) */
    div.stButton > button {
        width: 100%;
        height: 70px;
        font-size: 22px;
        ...
        /* HIỆU ỨNG 3D CỨNG (Solid Shadow) */
        box-shadow: 0 8px 0 rgba(0,0,0,0.2); 
    }

    /* Khi bấm nút thì nút lún xuống */
    div.stButton > button:active {
        top: 8px; /* Di chuyển xuống 8px */
        box-shadow: 0 0 0 rgba(0,0,0,0.2); /* Mất bóng đáy */
    }

    /* --- PHỐI MÀU NÚT --- */
    
    /* Nút 1 & 2 (Màu Tím & Xanh - Hàng trên) */
    div.stButton > button:nth-of-type(1) { background: linear-gradient(to bottom, #a55eea, #8854d0); }
    
    /* Các nút khác sẽ mặc định màu xanh dương/hồng tùy vị trí, nhưng ta sẽ dùng style inline nếu cần */
...
if st.session_state.step == 1:
    ...
    with c2:
        # Nút Bắt đầu (Màu Xanh lá - Custom Style)
        st.markdown("""
        <style>div.stButton > button {background: linear-gradient(to bottom, #2ecc71, #27ae60) !important;}</style>
        """, unsafe_allow_html=True)
...
# --- BƯỚC 2: HỌC SỐ (CÓ NÚT NGHE CÂU HỎI) ---
    ...
    c1, c2 = st.columns(2)
    with c1:
        # Style riêng cho nút Nghe câu hỏi (Màu Tím)
    ...
    with c2:
        # Style nút Nghe tên số (Màu Xanh Dương)
